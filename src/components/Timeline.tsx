...
function getMonsterImageFilename(monsterName: string) {
	return (
		monsterName.toLowerCase().replace('a ', '').replace('an ', '') + '.gif'
	);
}
...
function TimelineItem({ date, text, deathType, killer }) {
	...
	const monsterImageUrl = `/images/monsters/${getMonsterImageFilename(
		killer.killed_by
	)}`;
					...
					>
						<Circle size='64px' bg='#b794f4' position='relative'>
							<Image
								...
								alt={`${killer.killed_by}'s Outfit`}
								height={killer.isMonster ? '42px' : '64px'}
								width={killer.isMonster ? '42px' : '64px'}
								objectFit='contain'
								position='absolute'
								top='50%'
								left='50%'
